...
export default function DSA() {
  ...
  // Data
  const { data, isLoading, isError, error, refetch } = useDSAQuestions(
    {},
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-cyan-50">
      {/* Compact Hero */}
      <header className="relative bg-gradient-to-br from-brand-primary to-brand-secondary">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs md:text-sm text-white/90">
                    Curated interview problems — quickly filter, practice and
                    submit.
                  </p>
                </div>
              </div>
        ...
        <div className="grid lg:grid-cols-[300px_1fr] gap-6 h-full items-start">
          ...
          <aside className="hidden lg:block h-full">
            <div className="sticky top-6 h-full bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
              ...
              <div className="overflow-y-auto p-3 h-[calc(100% - 72px)]">
                {" "}
                  ...
                  {tags.map((tag) => (
                    <motion.button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-lg font-medium transition-all duration-200 flex items-center justify-between ${
                        selectedTag === tag
                          ? "text-white shadow-md bg-gradient-to-r from-brand-primary to-brand-secondary"
              ...
              <div className="flex items-center gap-3">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    className="pl-10 py-2 rounded-[10px]"
                  />
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-gray-100 h-full overflow-hidden">
                <AnimatePresence mode="popLayout">
                  {isLoading ? (
                    ...
                    >
                      <div
                        className="w-10 h-10 mx-auto mb-3 rounded-full bg-white/40 flex items-center justify-center"
                        style={{
                          boxShadow: "inset 0 2px 6px rgba(0,0,0,0.06)",
                        }}
                      >
                        <svg
                          className="animate-spin w-5 h-5 text-brand-secondary"
                          viewBox="0 0 50 50"
                        >
                          <circle
                            cx="25"
...
function MiniStat({ label, value, color = "blue" }) {
  ...
  return (
    <div className="flex flex-col items-start px-3 py-1 rounded-md bg-white/10">
      ...
      <div
        className={`text-sm font-bold mt-0.5 bg-clip-text text-transparent bg-gradient-to-r ${colorMap[color]}`}
      >
        {value}
      </div>
    </div>
    ...
    >
      <div
        className={`absolute inset-y-0 left-0 w-1 transition-all duration-200 ${
          isHovered
            ? "bg-gradient-to-r from-brand-primary to-brand-secondary opacity-100"
      ...
      <div className="p-3 pl-5 hover:bg-gradient-to-r hover:from-slate-50 hover:to-transparent transition-all duration-200">
        <div className="flex items-center gap-3">
          <div className="flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center text-white font-semibold text-sm shadow-sm bg-gradient-to-b from-brand-primary to-brand-secondary">
            {index}
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start gap-2">
              ...
              <span
                className={`px-2 py-0.5 rounded-md text-xs font-semibold border ${diff.bg} ${diff.text} ${diff.border} ml-1`}
              >
              ...
              {company.length > 0 && (
                <div className="ml-2 flex items-center gap-1">
                  {company.map((c, i) => (
                    <span
                      key={i}
                      className="px-2 py-0.5 bg-indigo-50 text-indigo-700 rounded-md text-xs font-medium border border-indigo-100"
                    >
                      {c}
                    </span>
                  ))}
          ...
          <ChevronRight
            className={`w-5 h-5 transition-all duration-200 ${
              isHovered
                ? "opacity-100 translate-x-0 text-brand-primary"
                : "opacity-0 -translate-x-2 text-transparent"
            }`}
          />
        </div>
      </div>
